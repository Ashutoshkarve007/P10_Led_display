...
void simgpstest()
{
  if (Serial.available())
  {
    ...
    Serial2.print("AT+CPIN\r");
    delay(500);
    while (Serial2.available())
    {
      simData = (char)Serial2.read();
      Serial.write((char)simData);
...
void switchcommands()
{
 ...
 // Serial.println("Data : " + f);
  ...
  else if (ani == 4)                                                     //Scroll from top to bottom
  {
    ...
    {
      fontmode();
      dmd.drawString(x, b, Data, strlen(Data), g);
      delay(spd);
    }
  }
  ...
  {
    dmd.clearScreen( true );
    for (int c = 32 ; c > b - 1; c++)
    {
      dmd.clearScreen( true );
      dmd.drawString(x, c, Data, strlen(Data), g);
      delay(spd);
    }
  }
